<template>
  <section>
    ...
    >
      ...
      <div
        class="mainForm w-full rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0"
        v-else
      >
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
          <h1
            class="text-xl text-center font-bold leading-tight tracking-tight md:text-2xl"
          >
            انشاء حساب
          </h1>
          <form @submit.prevent="submitData" class="space-y-3">
            ...
            <div>
              ...
              <input
                ...
                required=""
              />
              <p class="text-red-800 py-2 font-bold mx-3" v-if="!pass.valid">
                {{ pass.error }}
              </p>
            </div>
            <div>
              <label
                ...
                class="block mb-2 text-sm font-mediumdark:text-white"
                >تاكيد كلمة المرور
              </label>
              <input
                @blur="ClearValidation('confirmPass')"
                type="password"
                name="confirm-password"
                id="confirm-password"
                placeholder="••••••••"
                class="bg-gray-50 border border-gray-300 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500"
                v-model.trim="confirmPass.val"
                required=""
              />
              <p
                class="text-red-800 py-2 font-bold mx-3"
                v-if="!confirmPass.valid"
              >
                {{ confirmPass.error }}
              </p>
            </div>
            ...
            <button
              type="submit"
              class="w-full bg-white text-[#313A49] bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
            >
              انشاء حساب
            </button>
  ...
  methods: {
    ...
    isValidForm() {
      ...
      if (this.pass.val === "" || this.pass.val.length < 5) {
        this.pass.error =
          "لا يمكن ان يترك فارغ او عدد الاحرف اقل من 5 و يجب ان يحتوى على احرف كبيره و صغيره و علامات خاصة";
        this.validForm = false;
        this.pass.valid = false;
      }
      if (this.confirmPass.val === "" || this.confirmPass.val.length < 5) {
        this.confirmPass.error =
          "لا يمكن ان يترك فارغ او عدد الاحرف اقل من 5 و يجب ان يحتوى على احرف كبيره و صغيره و علامات خاصة";
        this.validForm = false;
